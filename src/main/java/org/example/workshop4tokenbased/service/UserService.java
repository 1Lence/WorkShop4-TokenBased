...
import org.example.workshop4tokenbased.security.jwt.JwtService;
...
public class UserService {
    ...
    public JwtAuthDto refreshToken(RefreshTokenDto refreshTokenDto) throws AuthenticationException {
        ...
        if(refreshedToken != null && jwtService.validateJwtToken(refreshedToken)) {
            User user = findByLogin(jwtService.getLoginFromToken(refreshedToken));
            return jwtService.refreshBaseToken(user.getLogin(), refreshedToken);
        }
    ...
    public UserDtoResponse findUserByLogin(String login) {
        return userDtoMapper.toUserDtoResponse(userRepository.findByLogin(login)
    ...
    public UserDtoResponse setRole(UserDtoRequest userDtoRequest) {
        ...
        return userDtoMapper.toUserDtoResponse(userRepository.save(user));
    }
